...
\begin{enumerate}
  ...
  (Sec.~\ref{sec:results:fault}). We claim no novelty for the
  scheduling mechanism itself; the novelty lies in the actuator and its
  consequences.
...
with the floor's purpose: preventing excursions into the steep region
of the quality--compute curve.
...
Future work: generalizing the actuator claim to a second sampling-based
optimizer class (e.g., CEM), which would elevate the contribution from a
single-instance demonstration to a class-level statement about sampling
budgets as feedback-scheduling actuators; a validated fallback
