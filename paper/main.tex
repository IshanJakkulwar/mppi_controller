...
degrading quality --- together with an elementary analysis of the
resulting allocation map and a pre-registered, two-baseline,
two-regime experimental evaluation of what adaptation does and does not
buy. The analysis yields a deadline criterion that predicts, from
measured cost inflation alone, both the near-elimination of misses in
one regime and their persistence in the other.
...
\begin{enumerate}
  ...
  objective and a domain-specific safety-fallback detector, the latter
  validated in flight by fault injection
  (Sec.~\ref{sec:results:fault}). The novelty lies in the actuator, not
  the mechanism (Sec.~\ref{sec:related}).
  \item An elementary analysis of the resulting allocation map
  (Sec.~\ref{sec:analysis}) giving: a deadbeat, oscillation-free
  allocation property; an error-transfer bound relating budget
  variation to estimator noise; a deadline criterion
  $\tau_{t+1}/\hat{\tau}_t \le 1/\beta$ that supplies both a design
  rule for the margin and a quantitative account of \emph{why} misses
  are near-eliminated in one regime and merely reduced in the other;
  and a conditional bound on quality degradation under the floor. Each
  prediction is checked against the measured data.
  \item An offline characterization of MPPI's quality--compute trade-off
  (a Pareto sweep over $N$) that is used twice: it \emph{predicts} the
  closed-loop behavior of the deadline-only law --- adaptation operates
  on the flat region of the curve, which is precisely why large budget
  reductions cost little tracking quality --- and it is \emph{fed back
  into the controller} as a derived quality floor
  ($N_{\mathrm{floor}} = 150$, the smallest $N$ within one noise-floor
  $\sigma$ of maximum quality), yielding a scheduler that respects a
  timing constraint and a quality constraint simultaneously under an
  explicit lexicographic priority, evaluated as a pre-registered
  ablation (Sec.~\ref{sec:results:qf}).
...
give up (Fig.~\ref{fig:pareto}).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_pareto}
\caption{Quality--compute trade-off of MPPI (10 seeds per $N$,
standalone task): RMS tracking error (left axis, mean $\pm$ std) and
per-call compute cost (right axis) versus sample count. The curve is
steep below $N \approx 150$ and flat beyond; dotted lines mark the
adaptive scheduler's mean operating points in the two regimes.}
\label{fig:pareto}
\end{figure}
...
\emph{Pareto-informed quality floor (ablation).} The base law
\eqref{eq:budget} is deadline-only: nothing prevents it from allocating
deep into the steep region of the quality--compute curve. As a
pre-registered refinement, we close this loop by feeding the offline
characterization (Sec.~\ref{sec:pareto}) back into the allocator. Let
$N_{\mathrm{floor}}$ be the smallest swept $N$ whose predicted quality
is within one noise-floor $\sigma$ of $N_{\max}$'s
($N_{\mathrm{floor}} = 150$ here --- derived from the sweep, not
chosen). The extended law applies a lexicographic priority:
\emph{(i) deadline (hard)}, \emph{(ii) quality floor (soft)},
\emph{(iii) margin (best effort)}: if \eqref{eq:budget} allocates below
$N_{\mathrm{floor}}$ and the floor is still affordable within the
\emph{full} period ($N_{\mathrm{floor}}\, \hat{\tau}_t \leq T$), the
floor is allocated --- spending deadline \emph{margin}, not the
deadline, to protect quality; if the floor itself is unaffordable, it
yields to \eqref{eq:budget} and the consecutive-miss detector below is
the escape hatch. Sec.~\ref{sec:results:qf} evaluates this variant
(ADAPTIVE-QF) as an ablation arm.
...
consecutive misses it raises a fallback \emph{signal}. To be precise
about scope: the mechanism implemented and validated in this paper is
the detector --- trigger, signal, and automatic clearing on the next
compliant cycle. Acting on the signal (e.g., substituting the PID stage
of our stack for MPPI) is a system-integration policy that we specify
but deliberately leave outside the control path, because the correct
substitution policy is platform-dependent; no controller handoff occurs
in any experiment reported here. The threshold of two avoids flapping
on isolated timing spikes and is motivated by consecutive-miss
weakly-hard stability analyses~\cite{maggio2020control}. Under the comparison conditions of
...
The budget law is simple enough to admit an elementary analysis, which
we give because it explains --- rather than merely accompanies --- the
experimental behavior reported in Sec.~\ref{sec:results}. Throughout we
assume the linear cost model $c_t = N_t \tau_t$ (per-cycle cost
proportional to sample count, with $\tau_t$ the per-sample cost) that
motivates the estimator in the first place.

\emph{Proposition 1 (deadbeat allocation; no oscillation in $N$).}
Under the linear cost model, $\hat{\tau}_t$ in \eqref{eq:budget} is
independent of the allocation history: $c_j / N_j = \tau_j$ regardless
of $N_j$. Hence $N_{t+1}$ is a static function of measured per-sample
cost alone --- the recursion contains no $N_t$ term. Consequently, if
$\hat{\tau}$ is constant at $\tau$ with $\beta T/\tau \in [N_{\min},
N_{\max}]$, the scheduler attains $N^\star = \lfloor \beta T/\tau
\rfloor$ in a \emph{single} cycle and remains there; no
allocation-driven oscillation is possible, and the only lag is the
$w$-cycle window over which $\hat{\tau}$ absorbs a step change in cost.
This is why we report response latency as an engineering metric rather
than proving asymptotic convergence: there is no dynamic mode in $N$ to
converge. It predicts a response latency between one and $w = 3$
cycles, against the measured $1.0$--$1.4$ cycles
(Sec.~\ref{sec:results:20hz},~\ref{sec:results:40hz}).

\emph{Proposition 2 (error transfer and chatter bound).} From
\eqref{eq:budget}, $|N_t - N^\star| / N^\star = |\tau -
\hat{\tau}_t| / \hat{\tau}_t$: the \emph{relative} allocation error
equals the relative estimator error, independent of the absolute scale
of $N$. Since $\hat{\tau}_t$ averages $w$ samples, its relative
dispersion is $\sigma_\tau / (\tau \sqrt{w})$, so the expected
cycle-to-cycle variation obeys $\mathbb{E}|\Delta N| / N \approx
\sqrt{2}\, \sigma_\tau / (\tau\sqrt{w})$ --- small whenever per-sample
cost is stable, and reducible by enlarging $w$ at the price of slower
response. Measured: $\mathbb{E}|\Delta N|/N = 1.8\%$ (20\,Hz) and
$4.1\%$ (40\,Hz), consistent with estimator noise rather than
scheduling chatter.

\emph{Proposition 3 (deadline criterion; interpretation of $\beta$).}
Cycle $t{+}1$ meets its deadline iff $N_{t+1}\tau_{t+1} \le T$, i.e.\
(substituting \eqref{eq:budget}, away from the clamps) iff
\begin{equation}
\label{eq:criterion}
\frac{\tau_{t+1}}{\hat{\tau}_t} \;\le\; \frac{1}{\beta}.
\end{equation}
The margin is therefore not an arbitrary tuning constant: $1/\beta$ is
exactly the factor by which realized per-sample cost may exceed its
estimate before a miss occurs, so $\beta = 0.6$ buys tolerance to a
$67\%$ instantaneous cost inflation. This criterion predicts our
headline results. Measured median per-sample cost inflation between
idle and loaded operation was $1.46\times$ at 20\,Hz --- inside the
$1.667$ bound, predicting deadline satisfaction, against
$0.15 \pm 0.38$ misses per trial observed --- and $1.85\times$ at
40\,Hz, outside the bound, predicting occasional misses, against
$0.75 \pm 0.71$ observed. The criterion thus accounts both for the
near-elimination of misses and for their ordering across regimes.
Equivalently, it yields a design rule: to survive a contention regime
that inflates per-sample cost by a factor $\kappa$, choose $\beta \le
1/\kappa$.

\emph{Proposition 4 (bounded quality degradation under the floor).}
Let the offline characterization (Sec.~\ref{sec:pareto}) certify
$|J(N) - J(N_{\max})| \le \sigma$ for all $N \ge N_{\mathrm{floor}}$,
where $J$ is the quality metric and $\sigma$ the sweep's noise floor.
Then whenever the quality floor is affordable ---
$N_{\mathrm{floor}}\hat{\tau}_t \le T$, the binding case of
Sec.~\ref{sec:scheduler} --- the allocated budget satisfies $N_{t+1}
\ge N_{\mathrm{floor}}$ and hence incurs quality degradation bounded by
$\sigma$ \emph{by construction}. The floor thus converts an empirical
observation (curve flatness) into a conditional guarantee; when it is
unaffordable the guarantee lapses precisely as the floor yields, which
is the regime the consecutive-miss detector exists to flag.

These statements are elementary, and deliberately so: they characterize
a static allocation map rather than a dynamical system. They are not
substitutes for a closed-loop stability result coupling allocation to
vehicle dynamics, which would require a quality model of MPPI as a
function of $N$ beyond the empirical curve used here, and which we
...
$p < 10^{-3}$; Cliff's $\delta = -1.00$, i.e.\ complete separation ---
every adaptive trial had fewer misses than every Fixed-400 trial),
while steady-state tracking error was statistically indistinguishable
(Welch $p = 0.35$; $|\delta| \leq 0.30$ across all in-regime
comparisons). During the load window the
...
Fig.~\ref{fig:overlay20} overlays all trials.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_overlay_20hz}
\caption{Primary regime (20\,Hz): sample count, MPPI call time, and
tracking error over time, all trials overlaid by condition. The load
window (t+20--50\,s) is visible as the adaptive budget reduction and
the Fixed-400 call-time excursion toward the 50\,ms deadline.}
\label{fig:overlay20}
\end{figure}
...
recovered within $1.6$ cycles --- inside the one-to-$w$ cycle range
predicted by Proposition 1; $\overline{|\Delta N|} \approx 2$
(1.8\% of $N$, matching the estimator-noise scale of Proposition 2)
...
Fig.~\ref{fig:dn} shows the $\Delta N$ distributions.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_dn_hist}
\caption{Cycle-to-cycle budget change $\Delta N_t$ for the adaptive
scheduler in both regimes (log scale). Mass concentrates at small
$|\Delta N|$; the tails correspond to the intended load-window
transitions, not sustained oscillation.}
\label{fig:dn}
\end{figure}
...
Mann--Whitney Holm-adjusted $p < 0.005$, Cliff's $\delta = -1.00$
against all three, leave-one-out robust) at the full 40\,Hz rate. The failure of the constants scales with their size:
...
all leave-one-out subsets; Holm-adjusted $p = 0.031$; Cliff's
$\delta = -0.75$), while Welch is not ($p \in [0.047, 0.97]$). The per-trial data explain why: adaptive's
...
Fig.~\ref{fig:overlay40} overlays all trials.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_overlay_40hz}
\caption{Secondary regime (40\,Hz, 25\,ms deadline): the adaptive
scheduler re-regulates to the tighter budget ($N \approx 252$ idle,
$\approx 143$ under load) while every constant's call time sits at or
above the deadline during contention.}
\label{fig:overlay40}
\end{figure}

\subsection{Quality-floor ablation (ADAPTIVE-QF)}
\label{sec:results:qf}

The Pareto-informed variant of Sec.~\ref{sec:scheduler} was evaluated
as an additional arm ($n{=}8$ per regime) under interpretation rules
pre-registered before any trial ran. Both regime-level predictions were
confirmed. At 20\,Hz the floor never activated (the deadline law's
in-window allocation, $N \approx 290$, never approaches 150) and
ADAPTIVE-QF behaves as the base law. At 40\,Hz the floor \emph{binds}
exactly as predicted: the base law's in-window allocation ($\approx
143$) falls below the floor, and ADAPTIVE-QF pins $N = 150$ for 74\% of
load-window cycles (in-window mean 151, call time 15.5\,ms), spending
deadline margin to hold quality.

Two measured effects characterize the resulting deadline/quality
tension. First, the \emph{cost}: in-window deadline misses rise from
essentially zero (base adaptive) to $4.1$ per trial --- the misses
concentrate precisely where the floor is active, the mechanistic
signature of margin being spent (total miss rates remain $\leq 0.4\%$
of cycles, orders below any constant under regime mismatch; adaptive
vs.\ ADAPTIVE-QF on misses: Holm-adjusted $p < 0.01$, Cliff's
$\delta = -1.00$). A smaller elevation outside the load window, and at
20\,Hz where the floor never activates, is attributable to between-batch
(different-day) variability rather than the mechanism --- a limitation
of the ablation-arm design, which we report rather than absorb into the
mechanism claim. Second, the apparent \emph{benefit}: at 40\,Hz,
ADAPTIVE-QF's steady-state RMS distribution is markedly tighter
($0.097 \pm 0.005$\,m across 8 trials, no outliers) than base
adaptive's ($0.136 \pm 0.127$\,m, whose mean is inflated by the single
0.451\,m outlier of Sec.~\ref{sec:results:40hz}); central tendencies do
not differ significantly ($p \approx 0.4$). With $n{=}8$ this variance
tightening is suggestive rather than definitive, but it is consistent
with the floor's purpose and with Proposition 4: while the floor binds,
quality degradation is bounded by the sweep's noise floor by
construction, whereas the deadline-only law carries no such bound and
is free to excurse into the steep region of the curve.

\emph{Conflict demonstration.} The true conflict case --- the deadline
demanding less than the floor \emph{and} the floor being unaffordable
--- is unreachable via fair-share contention on this platform, so we
exercised it by fault injection at 40\,Hz with a stall window long
enough for consecutive stalled cycles. A single half-second episode
then exhibits every branch of the lexicographic law in sequence: the
first stalled cycle (75\,ms against the 25\,ms deadline) registers a
miss; on the second, the floor \emph{binds} ($N = 150$, still
affordable on the partially-inflated cost estimate) and the second
consecutive miss fires the fallback detector; on the third, the
inflated estimate makes the floor unaffordable and it \emph{yields}
($N = 51$, then $N_{\min} = 20$) --- correctly so, since no allocation
fits a 60\,ms stall into a 25\,ms period, which is precisely the
persistent-transient case the detector signal exists for; when the
stall window ends, the first compliant cycle clears the detector, and
recovery re-enters \emph{through the floor} ($20 \to 88 \to 150$ held,
then upward) to the nominal operating point within 0.5\,s
(Fig.~\ref{fig:conflict}). Peak tracking disturbance across the episode
was 0.12\,m. Trigger, binding, yielding, clearing, and floor-mediated
recovery are thus each observed in flight, in their specified priority
order.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_conflict}
\caption{Conflict episode (ADAPTIVE-QF, 40\,Hz, extended injected
stall): consecutive $\sim$75\,ms stalls against the 25\,ms deadline.
The budget descends to the quality floor (150, dotted), the detector
fires (stars), the floor yields when unaffordable ($N \to 51 \to
N_{\min}$), and recovery re-enters through the floor to the nominal
operating point within 0.5\,s.}
\label{fig:conflict}
\end{figure}
...
Because the budget law's margin prevented any fallback signaling under
the comparison conditions, we validated the detector directly: in
...
($N: 400 \to \approx 255$); the second consecutive miss raised the
fallback signal exactly at the specified threshold (logged and recorded
in the cycle telemetry); the stall window then expired, the signal
cleared on the next compliant cycle, and the budget recovered to
$N_{\max}$ within at most 10 cycles (0.5\,s). To state the scope of
this validation precisely: what is exercised is the \emph{detector} ---
trigger, signal, and clearing. No controller handoff to PID occurred or
was configured (Sec.~\ref{sec:scheduler}); the adaptive controller rode
through the stall on its own, with peak tracking disturbance of 0.13\,m
and both flights completing the circuit normally --- itself
informative: for transients of this magnitude and duration, the budget
cut alone suffices, and the signal exists for transients that persist.
Validating a substitution \emph{policy} (when to hand off, and when to
return) is future work. Fig.~\ref{fig:fault} shows one episode.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_fault}
\caption{Fault-injection episode (trial 1): the injected stall drives
call time to $\sim$89\,ms against the 50\,ms deadline; the first miss
cuts the budget, the second consecutive miss raises the fallback signal
(star), and the budget recovers to $N_{\max}$ within 0.5\,s.}
\label{fig:fault}
\end{figure}
...
objective and a safety-fallback detector, and validated it in staged,
pre-registered
...
Future work: a closed-loop guarantee coupling allocation to vehicle
dynamics --- our analysis characterizes a static allocation map, and a
stability result would require a model of MPPI quality as a function of
$N$ beyond the empirical curve used here; generalizing the actuator
claim to a second sampling-based optimizer class (e.g., CEM), which
would elevate the contribution from a single-instance demonstration to a
class-level statement about sampling budgets as feedback-scheduling
actuators; a validated fallback
\emph{substitution} policy (when to hand off, and when to return);
hardware-in-the-loop and onboard-GPU deployment, where per-sample cost
models must account for batch effects; closed-loop guarantees combining
utilization bounds with MPPI's convergence properties; and composition
with distribution-adaptive MPPI variants.
