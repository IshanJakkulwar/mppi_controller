...
\author{\IEEEauthorblockN{Vishal Jakkulwar}
...
other variable workloads. We formulate runtime MPPI sample-count selection
as an online computational resource allocation problem, using measured
per-cycle execution time to adapt computational effort while maintaining
closed-loop control deadlines, with an explicit safety fallback mechanism.
...
\begin{enumerate}
  \item A formulation of MPPI sample-count selection as online
  computational resource allocation, with a measured-execution-time
  budget law and an explicit deadline-satisfaction objective and safety
  fallback (Sec.~\ref{sec:method}).
...
We are not aware of prior work that treats MPPI's sample count as a
runtime-adaptive control variable driven by measured per-cycle execution
time with an explicit deadline-satisfaction objective, validated in
closed-loop UAV experiments. Our results connect the threads above: an
...
\end{equation}
Equation~\eqref{eq:budget} is a ratio law: it targets a compute
utilization of $\beta T$ per cycle regardless of \emph{why} per-sample
cost changed (CPU contention, frequency scaling, or a changed deadline),
which is what allows the same law to transfer across operating regimes
...
analyses~\cite{maggio2020control}. In all experiments reported here the
fallback was signaled but never needed to be engaged by the adaptive
controller.
...
\begin{table}[t]
\caption{Primary regime (20\,Hz, 50\,ms deadline), mean $\pm$ std over
$n{=}13$ trials per condition. Load window: cores 0--1, 6 threads,
30\,s.}
\label{tab:20hz}
\centering
\begin{tabular}{lccc}
\toprule
 & Adaptive & Constant-330 & Fixed-400 \\
\midrule
Deadline misses      & $0.15 \pm 0.38$  & $0.23 \pm 0.60$  & $18.0 \pm 4.1$ \\
Miss rate            & $0.0\%$          & $0.0\%$          & $1.0\%$ \\
SS-RMS error [m]     & $0.131 \pm 0.055$& $0.135 \pm 0.051$& $0.155 \pm 0.071$ \\
Mean $N$             & $365.1$          & $330$            & $400$ \\
Mean call [ms]       & $27.9$           & $27.1$           & $31.4$ \\
In-window call [ms]  & $30.0$           & $32.8$           & $40.6$ \\
$\overline{|\Delta N|}$ & $2.0$         & $0$              & $0$ \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{Adaptive vs.\ Constant-330.} Under the tested condition, the
predetermined constant budget performed equivalently to adaptation on
both deadline misses ($p = 0.70$) and tracking error ($p = 0.83$): a
well-chosen constant matches adaptation \emph{in the regime it is tuned
for}. Sec.~\ref{sec:discussion} discusses why this is expected on our
platform, and Sec.~\ref{sec:results:40hz} shows what the constant cannot
do.
...
\toprule
 & Adaptive & Constant-330 & Fixed-400 \\
\midrule
Deadline misses  & $0.75 \pm 0.71$ & $1048.9 \pm 5.2$ & $876.5 \pm 7.6$ \\
Miss rate        & $0.0\%$         & $30.5\%$         & $26.9\%$ \\
SS-RMS error [m] & $0.136 \pm 0.127$ & $0.095 \pm 0.005$ & $0.130 \pm 0.045$ \\
Mean $N$         & $220.5$         & $330$            & $400$ \\
Mean call [ms]   & $15.0$          & $22.0$           & $26.3$ \\
\bottomrule
\end{tabular}
\end{table}
...
($0.75 \pm 0.71$ per trial; vs.\ both constants: Welch $p < 10^{-3}$,
Mann--Whitney $p < 10^{-3}$) at the full 40\,Hz rate. The constant
...
(Sec.~\ref{sec:discussion}). Tests on steady-state RMS between adaptive
and Fixed-400 disagree (Welch $p = 0.90$; Mann--Whitney $p = 0.01$, an
$n{=}8$ rank artifact driven by one outlier trial); we report both and
claim no consistent tracking difference.
