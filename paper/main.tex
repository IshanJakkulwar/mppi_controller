...
\begin{enumerate}
  ...
  objective and a domain-specific safety fallback
  (Sec.~\ref{sec:method}). We claim no novelty for the scheduling
  mechanism itself; the novelty lies in the actuator and its
  consequences.
...
test (reported wherever the two test types disagree).
\todo{refresh Holm-adjusted values once the const-200 arm joins the
reported family}
...
all leave-one-out subsets), while Welch is not
($p \in [0.047, 0.97]$). The per-trial data explain why: adaptive's
